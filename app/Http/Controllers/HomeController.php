...
class HomeController extends Controller
{
    ...
    public function available(Request $request)
    {
      ...
      $validator = Validator::make($input, [
        ...
        'backdate' => 'notbackdate',
      ]);

      if ($validator->fails()) {
        if ($validator->errors()->first("departure") != null) {
          // return redirect(route('home'))->withErrors($validator->errors()->first("departure"))->withInput(Input::all());
          Alert::error($validator->errors()->first("departure"), 'Wrong')->persistent('Close');
          return redirect()->back();
        }
        elseif ($validator->errors()->first("from") != null) {
          Alert::error($validator->errors()->first("from"), 'Wrong')->persistent('Close');
          return redirect()->back();
        }
        elseif ($validator->errors()->first("to") != null) {
          Alert::error($validator->errors()->first("to"), 'Wrong')->persistent('Close');
          return redirect()->back();
        }
      }

      if ($input['departure'] < Carbon::now()) {

        Alert::error('Can\'t order tickets before today', 'Wrong')->persistent('Close');
        return redirect()->back();
      }
