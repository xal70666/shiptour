<div style="background-color: #f7f7f7; margin: 10px;">
  ...
  <p><b>Dear {{ $nama_order }},</b></p>
  <p>Terimakasih telah melakukan pembelian tiket snorkeling di <a href="JTIndonesia.com">JTIndonesia.com</a></p>
  <p>
    @foreach ($data_destinasi as $key => $value)
      {{$value->nama}}, {{$value->alamat}}
    @endforeach
    {{$departure}}
  </p>
  <p>Kami menunggu pembayaran anda , Mohon Transfer sejumlah IDR {{$ttl_harga}} dengan batas waktu pembayaran sampai : <b>{{date("d-M-Y, H:i", strtotime($batas_pembayaran))}}</b></p>
